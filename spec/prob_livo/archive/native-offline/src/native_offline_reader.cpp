                              ...
                              const NativeOfflineDispatch &dispatch)
{
  ...
  if (options.image_stride == 0) {
    std::fprintf(stderr, "[FAST-LIVO native offline] image_stride must be positive\n");
    return false;
  }
  ...
  std::printf(
      ...
      "ignored=%zu wall=%.3fs decode=%.3fs decode_failures=%zu speed=%.3fx\n",
      ...
      accounting_.image_decode_failures, accounting_.speed_factor);
  return true;
}
