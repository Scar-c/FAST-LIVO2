...
NativeBenchmarkProcessMonitor::~NativeBenchmarkProcessMonitor() { Stop(); }

bool NativeBenchmarkProcessMonitor::Start(const std::string &csv_path,
  ...
  stop_.store(false);
  thread_ = std::thread(&NativeBenchmarkProcessMonitor::Run, this);
  return true;
}

void NativeBenchmarkProcessMonitor::SetSensorTimestamp(double timestamp) {
  if (std::isfinite(timestamp)) sensor_timestamp_.store(timestamp);
}

void NativeBenchmarkProcessMonitor::Stop() {
...
void NativeBenchmarkProcessMonitor::Run() {
