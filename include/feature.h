...
struct Feature
{
  ...
  std::unique_ptr<VisualPatch> patch_owner_;
  ...
  Feature(VisualPoint *_point, std::unique_ptr<VisualPatch> _patch,
          const Vector2d &_px, const Vector3d &_f, const SE3 &_T_f_w,
          int _level)
      : type_(CORNER), px_(_px), f_(_f), T_f_w_(_T_f_w), mean_(0), score_(0),
        level_(_level), patch_owner_(std::move(_patch)),
        patch_(patch_owner_ == nullptr ? nullptr : patch_owner_->data()),
        point_(_point)
  {
  }

  // Kept only for the Prompt20 V0 control path.  V1 and later must transfer
  // a VisualPatch owner instead of passing a raw temporary allocation.
  ...
  ~Feature()
  {
    if (patch_owner_ == nullptr) delete[] patch_;
  }
};
