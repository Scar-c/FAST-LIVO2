...
def associate_unique(est_times, gt_times, max_diff):
    """Return one-to-one pairs, prioritising the smallest time delta."""
    candidates = []
    for est_index, timestamp in enumerate(est_times):
        ...
        for gt_index in range(lo, hi):
            candidates.append(
                (abs(float(timestamp - gt_times[gt_index])), est_index,
                 gt_index)
            )
...
def umeyama_se3(source, destination):
    if source.shape != destination.shape or source.shape[0] < 3:
        raise ValueError("SE(3) alignment requires at least three paired points")
    source_mean = source.mean(axis=0)
    destination_mean = destination.mean(axis=0)
    covariance = ((source - source_mean).T @
                  (destination - destination_mean) / source.shape[0])
...
def parse_args(argv=None):
    ...
    parser.add_argument("--prism", type=parse_vector, default=np.zeros(3),
                        help="body-frame offset of compared GT point: x,y,z")
    parser.add_argument("--frame", default="body",
                        help="physical comparison frame")
    parser.add_argument("--max-diff", type=float, default=0.05,
                        help="maximum association delta in seconds")
    parser.add_argument("--min-matches", type=int, default=3)
    parser.add_argument("--out", help="optional persistent text output")
    return parser.parse_args(argv)


def evaluate(args):
    est_time, est_position, est_quaternion = load_tum(args.estimate)
    gt_time, gt_position, _ = load_tum(args.ground_truth)
    compared_position = np.empty_like(est_position)
    for index, (position, quaternion) in enumerate(
            zip(est_position, est_quaternion)):
        norm = np.linalg.norm(quaternion)
        if not np.isfinite(norm) or norm <= 0:
            raise ValueError(f"estimate quaternion {index} is invalid")
        compared_position[index] = (
            position + quat_to_rot(quaternion / norm) @ args.prism
        )
    pairs = associate_unique(est_time, gt_time, args.max_diff)
    ...
    matched_time = [est_time[est] for est, _, _ in pairs]
    assoc_dt = np.abs([gt_time[gt] - est_time[est]
                       for est, gt, _ in pairs])
    return {
        "matched": len(pairs),
        "duration": float(min(est_time[-1], gt_time[-1]) -
                          max(est_time[0], gt_time[0])),
        "matched_duration": float(matched_time[-1] - matched_time[0])
                            if len(matched_time) > 1 else 0.0,
        "assoc_dt_p50": float(np.percentile(assoc_dt, 50)),
        "assoc_dt_p90": float(np.percentile(assoc_dt, 90)),
        "assoc_dt_p95": float(np.percentile(assoc_dt, 95)),
        "assoc_dt_p99": float(np.percentile(assoc_dt, 99)),
        "assoc_dt_max": float(np.max(assoc_dt)),
