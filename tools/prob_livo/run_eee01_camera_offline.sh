...
if [[ -n "$(git -C "$REPO_ROOT" status --short)" ]]; then
  echo "ERR: offline runner requires a clean worktree" >&2
  exit 3
fi
...

{
  ...
  echo "prompt16_diagnostic_trace: ${PROB_LIVO_PROMPT16_TRACE_PATH:-none}"
...
if [[ "$NODE_RC" -eq 0 && "$COUNTER_RC" -eq 0 && "$GT_RC" -eq 0 && \
      "$EVAL_RC" -eq 0 ]]; then RC=0; else RC=1; fi
{
  ...
  echo "visual_lifecycle: $RUN_DIR/trajectory.tum.visual_lifecycle.csv"
...
echo "__I3_OFFLINE_RUN_DONE_RC=$RC"
exit "$RC"
