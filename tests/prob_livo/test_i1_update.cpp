...
struct ObservationSequence {
  ...
  void Fill(const StatesGroup &state, Matrix6 &ht_vinv_h,
  ...
  void FillOracle(const OracleState &state, prob_livo_test_oracle::M6 &ht_vinv_h,
...
int RunUpdateTests(TestContext &context) {
  ...
  const bool updated = filter.UpdateObserve(
      [&production_observations](const StatesGroup &state, Matrix6 &info,
                                 prob_livo::Vector6 &rhs) {
        production_observations.Fill(state, info, rhs);
      });
  const auto oracle_stats = prob_livo_test_oracle::UpdateObserve(
      oracle, oracle_options,
      [&oracle_observations](const OracleState &state,
                             prob_livo_test_oracle::M6 &info,
                             prob_livo_test_oracle::V6 &rhs) {
        oracle_observations.FillOracle(state, info, rhs);
      });
  ...
  long_filter.UpdateObserve(
      [&long_production_observations](const StatesGroup &state, Matrix6 &info,
                                      prob_livo::Vector6 &rhs) {
        long_production_observations.Fill(state, info, rhs);
      });
  const auto long_stats = prob_livo_test_oracle::UpdateObserve(
      long_oracle, long_oracle_options,
      [&long_oracle_observations](const OracleState &state,
                                  prob_livo_test_oracle::M6 &info,
                                  prob_livo_test_oracle::V6 &rhs) {
        long_oracle_observations.FillOracle(state, info, rhs);
      });
  ...
  converging_filter.UpdateObserve(
      [&converging_calls](const StatesGroup &, Matrix6 &info,
                          prob_livo::Vector6 &rhs) {
        info.setZero();
        rhs.setZero();
        ++converging_calls;
